...
const Movie: FC = () => {
  ...
  return (
    <Layout title={`${movie.title} (${movie.type} ${movie.year})`}>
      ...
      {
        showModal.showMovieInfoModal && createAppPortal(
          <MovieInfoModal creators={movie.creators} movieTitle={movie.title}
                          closeCallback={() => dispatch(setShowModal({...showModal, showMovieInfoModal: false}))}/>
        )
      }
    </Layout>
  );
